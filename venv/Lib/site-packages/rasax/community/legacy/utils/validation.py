...
def validate_yaml_schema(yaml_file_content: Text, schema_path: Text) -> None:
    ...
    try:
        ...
        # right line
        source_data = rasax.community.legacy.utils.io.read_yaml(
            yaml_file_content, reader_type=["safe", "rt"]
        )
    except (YAMLError, DuplicateKeyError) as e:
        raise YamlSyntaxException(underlying_yaml_exception=e)

    schema_file = pkg_resources.resource_filename(rasax.community.__name__, schema_path)
    schema_utils_file = pkg_resources.resource_filename(
        rasax.community.__name__, RESPONSES_SCHEMA_FILE
    )
    schema_extensions = pkg_resources.resource_filename(
        rasax.community.__name__, SCHEMA_EXTENSIONS_FILE
    )

    # Load schema content using our YAML loader as `pykwalify` uses a global instance
    # which can fail when used concurrently
    schema_content = rasax.community.legacy.utils.io.read_yaml_file(schema_file)
    schema_utils_content = rasax.community.legacy.utils.io.read_yaml_file(
        schema_utils_file
    )
