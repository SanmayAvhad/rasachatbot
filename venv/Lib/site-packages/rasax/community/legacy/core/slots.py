...
from rasax.community.legacy.constants import DOCS_URL_SLOTS
...
class Slot:
    ...
    def resolve_by_type(type_name: Text) -> Type["Slot"]:
        ...
        for cls in rasax.community.legacy.utils.common.all_subclasses(Slot):
            if cls.type_name == type_name:
                return cls
        try:
            return rasax.community.legacy.utils.common.class_from_module_path(type_name)
    ...
    def persistence_info(self) -> Dict[str, Any]:
        # TODO: Current 'replace' fixrequired for Rasa-OSS model parsing.
        # Changes in Rasa-OSS are required to no longer be aware of "rasa.shared" path.
        return {
            "type": rasax.community.legacy.utils.common.module_path_from_instance(
                self
            ).replace("rasax.community.legacy.core.slots", "rasa.shared.core.slots"),
...
class CategoricalSlot(Slot):
    ...
    def add_default_value(self) -> None:
        ...
        if (
            rasax.community.legacy.core.constants.DEFAULT_CATEGORICAL_SLOT_VALUE
            not in values
        ):
            self.values.append(
                rasax.community.legacy.core.constants.DEFAULT_CATEGORICAL_SLOT_VALUE
