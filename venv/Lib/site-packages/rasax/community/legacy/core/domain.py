...
from rasax.community.legacy.constants import (
...
from rasax.community.legacy.core.constants import (
    SlotMappingType,
    MAPPING_TYPE,
    MAPPING_CONDITIONS,
)

from rasax.community.legacy.exceptions import (
    RasaException,
    YamlException,
    YamlSyntaxException,
)
import rasax.community.legacy.utils.validation
import rasax.community.legacy.core.slot_mappings
import rasax.community.legacy.utils.io
import rasax.community.legacy.utils.common
from rasax.community.legacy.core.events import SlotSet, UserUttered
from rasax.community.legacy.core.slots import (
    Slot,
    CategoricalSlot,
    TextSlot,
    AnySlot,
...
from rasax.community.legacy.constants import RESPONSE_CONDITION
from rasax.community.legacy.nlu.constants import (
...
if TYPE_CHECKING:
    from rasax.community.legacy.core.trackers import DialogueStateTracker
...
class Domain:
    ...
    def from_file(cls, path: Text) -> "Domain":
        ...
        return cls.from_yaml(rasax.community.legacy.utils.io.read_file(path), path)
    ...
    def from_yaml(cls, yaml: Text, original_filename: Text = "") -> "Domain":
        ...
        try:
            ...
            data = rasax.community.legacy.utils.io.read_yaml(yaml)
            if not rasax.community.legacy.utils.validation.validate_training_data_format_version(
    ...
    def from_dict(cls, data: Dict) -> "Domain":
        ...
        rasax.community.legacy.core.slot_mappings.validate_slot_mappings(domain_slots)
        ...
        for slot_name in slot_dict:
            ...
            if slot_type:
                slot_type = slot_type.replace(
                    "rasa.shared.core.slots", "rasax.community.legacy.core.slots"
                )
    ...
    def _transform_intent_properties_for_internal_use(
        ...
        if properties:
            ...
            raise InvalidDomain(
                ...
                f" {rasax.community.legacy.constants.DOCS_URL_DOMAINS} for more information on how"
                f" to correctly add `intents` in the `domain` and"
                f" {rasax.community.legacy.constants.DOCS_URL_INTENTS} for examples on"
        ...
        if explicitly_included and ambiguous_entities:
            rasax.community.legacy.utils.io.raise_warning(
    ...
    @rasax.community.legacy.utils.common.lazy_property
                ...
                for _entity, sub_labels in entity.items():
                    ...
                    if sub_labels:
                        ...
                        raise InvalidDomain(
                            ...
                            f" {rasax.community.legacy.constants.DOCS_URL_DOMAINS} for more"
                            f" information on how to correctly add `entities` in the"
                            f" `domain` and {rasax.community.legacy.constants.DOCS_URL_ENTITIES}"
    ...
    def _add_default_intents(
        ...
        for intent_name in rasax.community.legacy.core.constants.DEFAULT_INTENTS:
    ...
    def _collect_overridden_default_intents(
        ...
        return sorted(
            intent_names.intersection(
                set(rasax.community.legacy.core.constants.DEFAULT_INTENTS)
            )
        )
    ...
    def fingerprint(self) -> Text:
        ...
        self_as_dict[
            KEY_INTENTS
        ] = rasax.community.legacy.utils.common.sort_list_of_dicts_by_first_key(
            self_as_dict[KEY_INTENTS]
        )
        self_as_dict[KEY_ACTIONS] = self.action_names_or_texts
        return rasax.community.legacy.utils.io.get_dictionary_fingerprint(self_as_dict)
    ...
    def _add_requested_slot(self) -> None:
        ...
        if (
            self.form_names
            and rasax.community.legacy.core.constants.REQUESTED_SLOT
            not in [slot.name for slot in self.slots]
        ):
            self.slots.append(
                TextSlot(
                    rasax.community.legacy.core.constants.REQUESTED_SLOT,
                    mappings=[],
    ...
    def _add_knowledge_base_slots(self) -> None:
        ...
        """
        if (
            rasax.community.legacy.core.constants.DEFAULT_KNOWLEDGE_BASE_ACTION
            ...
            knowledge_base_slots = [
                rasax.community.legacy.core.constants.SLOT_LISTED_ITEMS,
                rasax.community.legacy.core.constants.SLOT_LAST_OBJECT,
                rasax.community.legacy.core.constants.SLOT_LAST_OBJECT_TYPE,
            ]
    ...
    def _add_session_metadata_slot(self) -> None:
        self.slots.append(
            AnySlot(
                rasax.community.legacy.core.constants.SESSION_START_METADATA_SLOT,
                mappings=[],
            )
        )
    ...
    def concatenate_entity_labels(
        ...
        if entity:
            return [
                f"{entity}"
                f"{rasax.community.legacy.core.constants.ENTITY_LABEL_SEPARATOR}"
                f"{sub_label}"
    ...
    def _get_featurized_entities(self, latest_message: UserUttered) -> Set[Text]:
        ...
        entity_names_roles = set(
            f"{entity['entity']}"
            f"{rasax.community.legacy.core.constants.ENTITY_LABEL_SEPARATOR}{entity['role']}"
            for entity in entities
            if "entity" in entity.keys() and "role" in entity.keys()
        )
        entity_names_groups = set(
            f"{entity['entity']}"
            f"{rasax.community.legacy.core.constants.ENTITY_LABEL_SEPARATOR}{entity['group']}"
            for entity in entities
    ...
    def _get_slots_sub_state(
        ...
        for slot_name, slot in tracker.slots.items():
            ...
            if slot is not None and slot.as_feature():
                ...
                if (
                    slot.value
                    == rasax.community.legacy.core.constants.SHOULD_NOT_BE_SET
                ):
                    slots[
                        slot_name
                    ] = rasax.community.legacy.core.constants.SHOULD_NOT_BE_SET
    ...
    def _get_active_loop_sub_state(tracker: "DialogueStateTracker") -> Dict[Text, Text]:
        ...
        active_loop: Optional[Text] = tracker.active_loop.get(
            rasax.community.legacy.core.constants.LOOP_NAME
        )
        if active_loop:
            return {rasax.community.legacy.core.constants.LOOP_NAME: active_loop}
        else:
            return {}
    ...
    def get_active_state(
        ...
        """
        state = {
            rasax.community.legacy.core.constants.USER: self._get_user_sub_state(
                tracker
            ),
            rasax.community.legacy.core.constants.SLOTS: self._get_slots_sub_state(
                tracker, omit_unset_slots=omit_unset_slots
            ),
            rasax.community.legacy.core.constants.PREVIOUS_ACTION: self._get_prev_action_sub_state(
                tracker
            ),
            rasax.community.legacy.core.constants.ACTIVE_LOOP: self._get_active_loop_sub_state(
                tracker
            ),
        }
    ...
    def _remove_rule_only_features(
        ...
        rule_only_slots = rule_only_data.get(
            rasax.community.legacy.core.constants.RULE_ONLY_SLOTS, []
        )
        rule_only_loops = rule_only_data.get(
            rasax.community.legacy.core.constants.RULE_ONLY_LOOPS, []
        )

        # remove slots which only occur in rules but not in stories
        if rule_only_slots:
            for slot in rule_only_slots:
                state.get(rasax.community.legacy.core.constants.SLOTS, {}).pop(
                    slot, None
                )
        # remove active loop which only occur in rules but not in stories
        if (
            rule_only_loops
            and state.get(rasax.community.legacy.core.constants.ACTIVE_LOOP, {}).get(
                rasax.community.legacy.core.constants.LOOP_NAME
            )
            in rule_only_loops
        ):
            del state[rasax.community.legacy.core.constants.ACTIVE_LOOP]

    @staticmethod
    def _substitute_rule_only_user_input(state: State, last_ml_state: State) -> None:
        if not rasax.community.legacy.core.trackers.is_prev_action_listen_in_state(
            state
        ):
            if not last_ml_state.get(
                rasax.community.legacy.core.constants.USER
            ) and state.get(rasax.community.legacy.core.constants.USER):
                del state[rasax.community.legacy.core.constants.USER]
            elif last_ml_state.get(rasax.community.legacy.core.constants.USER):
                state[rasax.community.legacy.core.constants.USER] = last_ml_state[
                    rasax.community.legacy.core.constants.USER
                ]
        ...
        for tr, hide_rule_turn in tracker.generate_all_prior_trackers():
            ...
            if ignore_rule_only_turns:
                ...
                if last_ml_action_sub_state:
                    state[
                        rasax.community.legacy.core.constants.PREVIOUS_ACTION
    ...
    def persist_specification(self, model_path: Text) -> None:
        ...
        rasax.community.legacy.utils.io.create_directory_for_file(domain_spec_path)

        metadata = {"states": self.input_states}
        rasax.community.legacy.utils.io.dump_obj_as_json_to_file(
    ...
    def load_specification(cls, path: Text) -> Dict[Text, Any]:
        ...
        return json.loads(rasax.community.legacy.utils.io.read_file(metadata_path))
    ...
    def _transform_intents_for_file(
        ...
        for intent_name, intent_props in intent_properties.items():
            if (
                intent_name in rasax.community.legacy.core.constants.DEFAULT_INTENTS
            ...
            use_entities = set(
                ...
                if rasax.community.legacy.core.constants.ENTITY_LABEL_SEPARATOR
                not in entity
            )
    ...
    def cleaned_domain(self) -> Dict[Text, Any]:
        ...
        for slot in domain_data[KEY_SLOTS].values():
            ...
            # ATTENTION: This section deviates from the original Rasa Open Source code.
            # This change was made to ensure that we are able to resolve the
            # slot type to its simpler name in all cases
            slot_type = slot["type"].replace(
                "rasa.shared.core.slots.", "rasax.community.legacy.core.slots."
            )
            if slot_type.startswith("rasax.community.legacy.core.slots"):
                slot["type"] = Slot.resolve_by_type(slot_type).type_name
    ...
    def persist(self, filename: Union[Text, Path]) -> None:
        ...
        rasax.community.legacy.utils.io.write_text_file(as_yaml, filename)
    ...
    def as_yaml(self, clean_before_dump: bool = False) -> Text:
        ...
        return rasax.community.legacy.utils.io.dump_obj_as_yaml_to_string(
    ...
    def _actions_for_domain_warnings(self) -> List[Text]:
        ...
        return [
            ...
            if action not in rasax.community.legacy.core.constants.DEFAULT_ACTION_NAMES
        ]
    ...
    def _combine_user_with_default_actions(user_actions: List[Text]) -> List[Text]:
        ...
        return (
            rasax.community.legacy.core.constants.DEFAULT_ACTION_NAMES
            + unique_user_actions
        )
    ...
    def utterances_for_response(self) -> Set[Text]:
        ...
        for action in self.action_names_or_texts:
            if not action.startswith(rasax.community.legacy.constants.UTTER_PREFIX):
                continue
    ...
    def is_domain_file(filename: Union[Text, Path]) -> bool:
        ...
        """
        from rasax.community.legacy.data import is_likely_yaml_file

        if not is_likely_yaml_file(filename):
            return False

        try:
            content = rasax.community.legacy.utils.io.read_yaml_file(filename)
