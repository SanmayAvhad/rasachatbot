...
from rasax.community.legacy.core.training_data.story_reader.markdown_story_reader import (
    MarkdownStoryReader,
)
...
from rasax.community.legacy.data import YAML_FILE_EXTENSIONS, MARKDOWN_FILE_EXTENSIONS
...
def _get_reader(
    ...
    if rasax.community.legacy.data.is_likely_markdown_file(filename):
...
async def load_data_from_resource(
    ...
    return await load_data_from_files(
        rasax.community.legacy.utils.io.list_files(resource),
        domain,
        template_variables,
        use_e2e,
