...
from rasax.community.legacy.core.slots import TextSlot, ListSlot
from rasax.community.legacy.exceptions import YamlException
import rasax.community.legacy.utils.io
from rasax.community.legacy.core.constants import LOOP_NAME
...
from rasax.community.legacy.nlu.training_data import entities_parser
...
from rasax.community.legacy.core.constants import RULE_SNIPPET_ACTION_NAME
from rasax.community.legacy.core.events import UserUttered, SlotSet, ActiveLoop
from rasax.community.legacy.core.training_data.story_reader.story_reader import (
    StoryReader,
)
from rasax.community.legacy.core.training_data.structures import StoryStep
...
CORE_SCHEMA_FILE = "legacy/utils/schemas/stories.yml"
...
class YAMLStoryReader(StoryReader):
    ...
    def from_reader(cls, reader: "YAMLStoryReader") -> "YAMLStoryReader":
        ...
        return cls(
            reader.domain,
            reader.source_name,
            # ATTENTION: This is a change from the Rasa Open Source code.
            # This change was made to be able to suppress that Rasa expands stories with
            # OR statements as this would lead to huge diffs when Rasa Enterprise dumps
            # stories back to disk.
            expand_or_statements=reader._expand_or_statements,
        )
    ...
    def read_from_file(
        ...
        try:
            return self.read_from_string(
                rasax.community.legacy.utils.io.read_file(
                    filename, rasax.community.legacy.utils.io.DEFAULT_ENCODING
                ),
                skip_validation,
            )
    ...
    def is_stories_file(cls, file_path: Union[Text, Path]) -> bool:
        ...
        """
        return rasax.community.legacy.data.is_likely_yaml_file(
            file_path
        ) and rasax.community.legacy.utils.io.is_key_in_yaml(
    ...
    def _parse_or_statement(self, step: Dict[Text, Any]) -> None:
        ...
        if events:
            self.current_step_builder.add_events(
                events, expand_or_statements=self._expand_or_statements
            )
