...
import rasax.community.legacy.core.events
from rasax.community.legacy.core.events import (
    UserUttered,
    ActionExecuted,
    SlotSet,
    ActiveLoop,
...
from rasax.community.legacy.core.training_data.story_writer.story_writer import (
    StoryWriter,
)
from rasax.community.legacy.core.training_data.structures import (
    StoryStep,
    Checkpoint,
    STORY_START,
    RuleStep,
...
class YAMLStoryWriter(StoryWriter):
    ...
    def process_event(self, event: Event) -> Optional[OrderedDict]:
        if isinstance(event, list):
            ...
            # This change was made to be able to dump stories which don't have expanded
            # or-statements. Rasa Enterprise avoids expanding as this would lead to
            # huge diffs when Rasa Enterprise dumps
            # stories back to disk.
            return self.process_or_statements(event)
    ...
    def process_user_utterance(
        ...
        if user_utterance.text and (
            ...
            result[KEY_USER_MESSAGE] = LiteralScalarString(
                rasax.community.legacy.core.events.format_message(
                    user_utterance.text,
    ...
    def process_action(action: ActionExecuted) -> Optional[OrderedDict]:
        ...
        if (
            action.action_name
            == rasax.community.legacy.core.constants.RULE_SNIPPET_ACTION_NAME
        ):
            return None
    ...
    def process_or_statements(self, events: List[UserUttered]) -> OrderedDict:
        """Converts unexpanded or-statements to YAML format.

        Args:
            events: Events belonging to the same `OR` statement.
        ...
        return OrderedDict(
            [
                (
                    ...
                    [
                        # ATTENTION: This is a change from the Rasa Open Source code.
                        # This change was made to be able to dump stories which don't
                        # have expanded or-statements. Rasa Enterprise avoids
                        # expanding as this would lead to huge diffs when Rasa
                        # Enterprise dumps stories back to disk.
