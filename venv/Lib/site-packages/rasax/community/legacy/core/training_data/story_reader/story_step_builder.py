...
import rasax.community.legacy.core.training_data.structures
import rasax.community.legacy.utils.io
from rasax.community.legacy.constants import DOCS_URL_STORIES
from rasax.community.legacy.core.events import UserUttered, Event
...
class StoryStepBuilder:
    ...
    def add_events(
        ...
        expand_or_statements: bool = True,
    ) -> None:
        """Adds next story steps with the specified list of events.

        Args:
            events: Events that need to be added.
            expand_or_statements: If `False` this doesn't expand OR statements.
        """
        ...
        if len(events) == 1:
            ...
            # ATTENTION: This is a change from the Rasa Open Source code.
            # (see comment on the param for more details)

            # this simplifies conversion between formats, but breaks the logic
            if not expand_or_statements:
                for t in self.current_steps:
                    t.add_events(events)
                return
    ...
    def _generate_checkpoint_name_for_or_statement(self, events: List[Event]) -> str:
        ...
        hashed_id = rasax.community.legacy.utils.io.get_text_hash(unique_id)[
            :GENERATED_HASH_LENGTH
        ]
