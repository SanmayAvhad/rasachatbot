...
from rasax.community.legacy.constants import UTTER_PREFIX
...
def get_file_format_extension(resource_name: Text) -> Text:
    ...
    files = rasax.community.legacy.utils.io.list_files(resource_name)

    file_formats = list(map(lambda f: loading.guess_format(f), files))

    if not file_formats:
        return rasax.community.legacy.data.yaml_file_extension()

    known_file_formats = {
        loading.RASA_YAML: rasax.community.legacy.data.yaml_file_extension()
    }
    fformat = file_formats[0]
    if all(f == fformat for f in file_formats):
        return known_file_formats.get(
            fformat, rasax.community.legacy.data.yaml_file_extension()
        )
...
# Attention: The `scipy` type was dropped to avoid the import of this library
def sparse_matrix_to_string(m: Any) -> Text:
