...
from rasax.community.legacy.utils.common import lazy_property
...
from rasax.community.legacy.nlu.training_data import util
...
class TrainingData:
    ...
    def _load_lookup_table(lookup_table: Dict[Text, Any]) -> Dict[Text, Any]:
        ...
        if Path(potential_file).is_file():
            try:
                lookup_table["elements"] = rasax.community.legacy.utils.io.read_file(
                    potential_file
                )
                return lookup_table
    ...
    def fingerprint(self) -> Text:
        ...
        return rasax.community.legacy.utils.io.deep_container_fingerprint(
            relevant_attributes
        )
    ...
    def label_fingerprint(self) -> Text:
        ...
        return rasax.community.legacy.utils.io.deep_container_fingerprint(labels)
    ...
    def nlu_as_json(self, **kwargs: Any) -> Text:
        ...
        from rasax.community.legacy.nlu.training_data.formats import RasaWriter
    ...
    def nlg_as_yaml(self) -> Text:
        ...
        """
        from rasax.community.legacy.nlu.training_data.formats.rasa_yaml import (
            RasaYAMLWriter,
        )
    ...
    def persist_nlu(self, filename: Text = DEFAULT_TRAINING_DATA_OUTPUT_PATH) -> None:
        """Saves NLU to a file."""
        if rasax.community.legacy.data.is_likely_json_file(filename):
            rasax.community.legacy.utils.io.write_text_file(
                self.nlu_as_json(indent=2), filename
            )
        elif rasax.community.legacy.data.is_likely_yaml_file(filename):
            rasax.community.legacy.utils.io.write_text_file(
    ...
    def persist_nlg(self, filename: Text) -> None:
        """Saves NLG to a file."""
        if rasax.community.legacy.data.is_likely_yaml_file(filename):
            rasax.community.legacy.utils.io.write_text_file(
    ...
    def get_nlg_persist_filename(nlu_filename: Text) -> Text:
        ...
        if rasax.community.legacy.data.is_likely_json_file(nlu_filename):
            ...
            extension = rasax.community.legacy.data.yaml_file_extension()
