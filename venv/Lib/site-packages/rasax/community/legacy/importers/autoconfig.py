...
from rasax.community.legacy.exceptions import FileNotFoundException
import rasax.community.legacy.utils.cli
...
COMMENTS_FOR_KEYS = {
    "pipeline": (
        ...
        f"# # See {rasax.community.legacy.constants.DOCS_URL_PIPELINE} for more information.\n"
    ),
    "policies": (
        ...
        f"# # See {rasax.community.legacy.constants.DOCS_URL_POLICIES} for more information.\n"
    ),
}
...
def get_configuration(
    ...
    config = rasax.community.legacy.utils.io.read_model_configuration(config_file_path)
...
def _get_unspecified_autoconfigurable_keys(
    ...
    if training_type == TrainingType.NLU:
        all_keys = rasax.community.legacy.constants.CONFIG_AUTOCONFIGURABLE_KEYS_NLU
    elif training_type == TrainingType.CORE:
        all_keys = rasax.community.legacy.constants.CONFIG_AUTOCONFIGURABLE_KEYS_CORE
    else:
        all_keys = rasax.community.legacy.constants.CONFIG_AUTOCONFIGURABLE_KEYS
...
def _get_missing_config_keys(
    ...
    if training_type == TrainingType.NLU:
        all_keys = rasax.community.legacy.constants.CONFIG_KEYS_NLU
    elif training_type == TrainingType.CORE:
        all_keys = rasax.community.legacy.constants.CONFIG_KEYS_CORE
    else:
        all_keys = rasax.community.legacy.constants.CONFIG_KEYS
...
def _auto_configure(
    ...
    if keys_to_configure:
        logger.debug(
            ...
            f"{rasax.community.legacy.utils.common.transform_collection_to_sentence(keys_to_configure)}. "  # noqa: E501, W505
    ...
    default_config = rasax.community.legacy.utils.io.read_config_file(
        default_config_file
    )
...
def _dump_config(
    ...
    if not config_as_expected:
        rasax.community.legacy.utils.cli.print_error(
    ...
    current_config_content = rasax.community.legacy.utils.io.read_file(config_file_path)
    ...
    rasax.community.legacy.utils.io.write_text_file(
        "".join(updated_lines), config_file_path
    )

    auto_configured_keys = rasax.community.legacy.utils.common.transform_collection_to_sentence(
        auto_configured_keys
    )
    rasax.community.legacy.utils.cli.print_info(
...
def _is_config_file_as_expected(
    ...
    try:
        content = rasax.community.legacy.utils.io.read_config_file(config_file_path)
...
def _add_missing_config_keys_to_file(
    ...
    if not missing_keys:
        return
    with open(
        config_file_path, "a", encoding=rasax.community.legacy.utils.io.DEFAULT_ENCODING
    ) as f:
