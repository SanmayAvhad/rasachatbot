...
from rasax.community.legacy.importers.importer import TrainingDataImporter
from rasax.community.legacy.importers import utils
...
from rasax.community.legacy.utils.common import mark_as_experimental_feature
...
class MultiProjectImporter(TrainingDataImporter):
    def __init__(
        ...
        self.config = rasax.community.legacy.utils.io.read_model_configuration(
            config_file
        )
        if domain_path:
        ...
        extra_nlu_files = rasax.community.legacy.data.get_data_files(
            training_data_paths, rasax.community.legacy.data.is_nlu_file
        )
        extra_story_files = rasax.community.legacy.data.get_data_files(
    ...
    def _init_from_file(self, path: Text) -> None:
        ...
        if os.path.exists(path) and rasax.community.legacy.data.is_config_file(path):
            config = rasax.community.legacy.utils.io.read_config_file(path)
    ...
    def _is_in_imported_paths(self, path: Text) -> bool:
        return any(
            [
                rasax.community.legacy.utils.io.is_subdirectory(path, i)
                for i in self._imports
            ]
        )
