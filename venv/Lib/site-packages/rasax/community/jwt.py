...
def verify_bearer_token(
    ...
    try:
        return _decode_jwt(
            authorization_header_value, public_key or rasa_x_config.jwt_public_key
        )
    except Exception:
        raise ValueError("Invalid bearer token.")


def _decode_jwt(value: Text, public_key: Union[Text, bytes],) -> Dict:
    return jwt.decode(
        value,
        public_key or rasa_x_config.jwt_public_key,
...
def _verify_keys(private_key: bytes, public_key: bytes) -> None:
    ...
    _decode_jwt(encoded, public_key)
